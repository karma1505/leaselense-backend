...
LEASE_PATH = "demo_leases/Demo_Lease_2_Flawed.md"
...
def verify_lease():
    ...
    try:
        ...
        expected_issues = ["Escalation", "Inspection", "Termination", "Deposit"]
        
        for i, r in enumerate(risks, 1):
            print(f"{i}. Type: {r.get('risk_type')} | Found: {r.get('risk_found')}")
            print(f"   Expl: {r.get('explanation')}")
            print("-" * 40)
            
        print("\n[VERDICT]")
        if len(risks) >= 3:
             print("✅ PASSED: Detected multiple flaws as expected.")
        else:
             print("⚠️ WARNING: Might have missed some flaws.")
